...
fn main_impl() -> Result<(), Error> {
    ...
    let matches = App::new(env!("CARGO_PKG_NAME"))
        ...
        .subcommand(SubCommand::with_name("run")
                        ...
                        .arg(Arg::with_name("NO_SEND")
                                 .long("no-send")
                                 .help("Run as normal but do not send emails"))
                        .arg(Arg::with_name("FEED_URL")
                                 .multiple(true)
                                 .help("URL of the feed(s) to run")))
        .get_matches();
    ...
    if let Some(matches) = matches.subcommand_matches("add") {
        ...
        let sender = feed::EmailSender::new(&config)?
            .with_no_send(matches.is_present("NO_SEND"));
        let options = model::FetchAndSendOptions {
            feed_urls: matches
                .values_of("FEED_URL")
                .map(|x| x.map(|x| String::from(x)).collect()),
        };
        model
            .fetch_and_send_feeds(logger, fetcher, &sender, &options)?;
    } else {
        unreachable!();
    }
